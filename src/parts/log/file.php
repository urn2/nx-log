...
trait file{
	...
	protected function nx_parts_log_file(): \Generator{
		...
		if(!$deferred){
			$this->buffer['log/file']['file'] = @fopen($path . $name . '.log', 'ab');
		}
		yield;
		if(!$deferred){
			if($this->buffer['log/file']['file']) fclose($this->buffer['log/file']['file']);
		}
		else{
	...
	protected function log_file_writer($log): void{
		if($this->log_file_deferred ?? true){
			...
			if($this->buffer['log/file']['file']) fwrite($this->buffer['log/file']['file'], $this->log_file_line($log) . "\n");
		}
	}
}